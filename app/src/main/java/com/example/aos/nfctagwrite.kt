...
class nfctagwrite : AppCompatActivity() {
    ...
    private fun writeNdefMessageToTag(ndefMessage: NdefMessage, tag: Tag?) {
        try {
            ...
            Toast.makeText(this, "NDEF message written to NFC tag.", Toast.LENGTH_SHORT).show()
        } catch (e: IOException) {
            // Show a toast message if there was an error writing the NDEF message to the NFC tag
            Toast.makeText(this, "Error writing NDEF message to NFC tag.", Toast.LENGTH_SHORT).show()
        } catch (e: FormatException) {
            // Show a toast message if there was an error formatting the NDEF message
            Toast.makeText(this, "Error formatting NDEF message.", Toast.LENGTH_SHORT).show()
        }
    }
}